...
void callback(const Image::ConstPtr& depth, const PointCloud2::ConstPtr& velo_cloud, const CameraInfo::ConstPtr& info){ 
	...
	if (DEBUG) ROS_INFO("data type in depth image is %s %dx%d \n", ty.c_str(), depth_im.cols, depth_im.rows);
	double min_d, max_d;
	cv::minMaxLoc(depth_im, &min_d, &max_d);
	if (DEBUG) ROS_INFO("depth map : min is %f and max is %f", min_d, max_d);
...
//      		mY.at<float>(Y, X) = uv.y - (float)Y;
	...
	if (DEBUG) ROS_INFO("projected image : min is %f and max is %f", min_p, max_p);
	...
	//Creating dense map
//	cv::Mat mD_roi(mD, cv::Rect(0, Ymin-10, Width, (Height - Ymin + 10)));
  	velo_im = cv::Mat::zeros(depth_im.size(), depth_im.type());
//	cv::Mat velo_im_roi(velo_im, cv::Rect(0, Ymin-10, Width, (Height - Ymin + 10)));
//	SparseToDense(mX, mY, mD, grid, velo_im);
	...
	if (DEBUG) ROS_INFO("Velodyne image : min is %f and max is %f", min_v, max_v);
	...
	if (DEBUG) ROS_INFO("Total number points projected to camera frame is %ld and after operation is %d", nonzero.size(), count_nz);
